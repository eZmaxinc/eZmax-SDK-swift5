...
```swift
    open class func emailtypeGetAutocompleteV2(sSelector: SSelector_emailtypeGetAutocompleteV2, eFilterActive: EFilterActive_emailtypeGetAutocompleteV2? = nil, sQuery: String? = nil, acceptLanguage: HeaderAcceptLanguage? = nil, completion: @escaping (_ data: EmailtypeGetAutocompleteV2Response?, _ error: Error?) -> Void)
```
...
// Retrieve Emailtypes and IDs
ObjectEmailtypeAPI.emailtypeGetAutocompleteV2(sSelector: sSelector, eFilterActive: eFilterActive, sQuery: sQuery, acceptLanguage: acceptLanguage) { (response, error) in
