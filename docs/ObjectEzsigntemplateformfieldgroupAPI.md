...
```swift
    open class func ezsigntemplateformfieldgroupGetObjectV1(pkiEzsigntemplateformfieldgroupID: Int, completion: @escaping (_ data: EzsigntemplateformfieldgroupGetObjectV1Response?, _ error: Error?) -> Void)
```
...
// Retrieve an existing Ezsigntemplateformfieldgroup
ObjectEzsigntemplateformfieldgroupAPI.ezsigntemplateformfieldgroupGetObjectV1(pkiEzsigntemplateformfieldgroupID: pkiEzsigntemplateformfieldgroupID) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
