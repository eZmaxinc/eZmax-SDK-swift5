...
```swift
    open class func provinceGetAutocompleteV2(sSelector: SSelector_provinceGetAutocompleteV2, eFilterActive: EFilterActive_provinceGetAutocompleteV2? = nil, sQuery: String? = nil, acceptLanguage: HeaderAcceptLanguage? = nil, completion: @escaping (_ data: ProvinceGetAutocompleteV2Response?, _ error: Error?) -> Void)
```
...
// Retrieve Provinces and IDs
ObjectProvinceAPI.provinceGetAutocompleteV2(sSelector: sSelector, eFilterActive: eFilterActive, sQuery: sQuery, acceptLanguage: acceptLanguage) { (response, error) in
