...
```swift
    open class func companyGetAutocompleteV2(sSelector: SSelector_companyGetAutocompleteV2, eFilterActive: EFilterActive_companyGetAutocompleteV2? = nil, sQuery: String? = nil, acceptLanguage: HeaderAcceptLanguage? = nil, completion: @escaping (_ data: CompanyGetAutocompleteV2Response?, _ error: Error?) -> Void)
```
...
// Retrieve Companys and IDs
ObjectCompanyAPI.companyGetAutocompleteV2(sSelector: sSelector, eFilterActive: eFilterActive, sQuery: sQuery, acceptLanguage: acceptLanguage) { (response, error) in
